---
name: code-style
description: Use before writing, reviewing, or refactoring any non-trivial code — enforces architecture and separation-of-concerns standards, naming conventions, error handling, and code review criteria. Trigger on requests to "write a function/component/module," "refactor this," "review my code," or "clean this up," not just explicit quality/lint requests. Consult this even mid-task when adding new files or restructuring existing ones.
---
...
- self contained, ideally reusable pieces of business logic
- static methods of global objects are heavily discouraged
- dependencies as objects instead of static methods of global objects for mockup during testing
- template

```typescript
// Interface if service swap is possible
class Foo implements Interface {
	public constructor(/* dependencies *8*/)
	// implementation
	public doWork() {}
}
```

- reasons to extract services
    - Readability
    - Organization, focus
    - Save LLM context
    - Testability
...
- if di container is implemented in the application, register services in DI container
